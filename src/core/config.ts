...
export function buildConfig(custom: ClientConfig): NormalizedClientConfig {
    ...
    return {
        ...
        ...(custom.family !== undefined ? { family: custom.family } : {}),
        ...(custom.insecureHTTPParser !== undefined ? { insecureHTTPParser: custom.insecureHTTPParser } : {}),
        ...(custom.allowedSocketPaths ? { allowedSocketPaths: custom.allowedSocketPaths } : {}),
        ...(custom.sensitiveHeaders ? { sensitiveHeaders: custom.sensitiveHeaders } : {}),
        ...(custom.timeoutErrorMessage !== undefined ? { timeoutErrorMessage: custom.timeoutErrorMessage } : {}),
        ...(custom.redact ? { redact: custom.redact } : {}),
        ...(custom.formDataHeaderPolicy ? { formDataHeaderPolicy: custom.formDataHeaderPolicy } : {}),
        ...(custom.env ? { env: custom.env } : {}),
